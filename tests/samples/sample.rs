...
fn inv_shift_rows(state: &mut [[u8; 4]; 4]) {
    todo!("#321 Undescribed by author.");
}
...
// TODO #322: Find a way to XOR each byte of state with a round key
fn add_round_key(state: &mut [[u8; 4]; 4], round_key: [u32; 4]) {
    for c in 0..4 {
        todo!("#323 Undescribed by author.")
    }
}
...
fn print_state(state: &[[u8; 4]; 4]) {
    todo!("#325 Undescribed by author.");
}
...
impl AesCtrSecret {
    ...
    pub fn encrypt<'py>(&self, py: Python<'py>, data: &PyByteArray) -> PyResult<&'py PyBytes> {
        todo!("#326 Undescribed by author."); // With inline comment...
    }
}
